...
def main(page: ft.Page):
    ...
    def cliente_especifico(e):
        dni_cliente = ft.TextField(label="DNI", icon= ft.Image(src='assets/lupa.png', width=28, height=28))

        def buscar_cliente(e):
                # Limpiar antes de mostrar nuevos datos

            query = "SELECT * FROM Clientes WHERE DNI=%s"
            db.cursor.execute(query, (dni_cliente.value,))
            resultados = db.cursor.fetchall()
